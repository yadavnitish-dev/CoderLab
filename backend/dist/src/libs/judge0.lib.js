...
export const buildBatchedStdin = (testcases) => [
...
export const parseBatchedStdout = (output) => {
    if (typeof output !== "string") {
        return null;
    }
    const pattern = new RegExp(`${CASE_OUTPUT_START}\\n([\\s\\S]*?)\\n${CASE_OUTPUT_END}`, "g");
...
export const executeSubmission = async (submission) => {
...
export const getJudge0LanguageId = (language) => LANGUAGE_ID_BY_NAME[language.toUpperCase()];
...
const ensureJDoodleCredentials = () => {
    if (!process.env.JDOODLE_CLIENT_ID || !process.env.JDOODLE_CLIENT_SECRET) {
        throw new Error("JDOODLE_CLIENT_ID and JDOODLE_CLIENT_SECRET are not defined");
    }
};
...
const executeJDoodle = async (submission) => {
    ...
    const { data } = await axios.post(JDOODLE_EXECUTE_URL, payload, {
        timeout: 20000,
        headers: {
...
const mapJDoodleToJudge0 = (jdoodle, token) => {
    const status = inferStatus(jdoodle);
    const compileOutput = status.description === "Compilation Error"
        ? toNullableString(jdoodle.error ?? jdoodle.output)
        : null;
    const runtimeError = status.description === "Runtime Error" ||
