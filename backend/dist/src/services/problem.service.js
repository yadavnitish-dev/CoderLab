...
import { buildBatchedStdin, executeSubmission, getJudge0LanguageId, } from "../libs/judge0.lib.js";
import { splitExecutionOutput } from "../libs/output.util.js";
import { ValidationError, NotFoundError, ForbiddenError, InternalServerError, } from "./errors.js";
/**
 * Problem Service
...
export class ProblemService {
     ...
     */
    async createProblem(userId, input) {
        // Validate input
        this.validateProblemInput(input);
        // Validate reference solutions
        await this.validateReferenceSolutions(input.testcases, input.referenceSolutions);
        // Create problem
     ...
     */
    async getAllProblems(page = 1, limit = 20) {
     ...
     */
    async updateProblem(problemId, userId, input) {
        if (!problemId) {
            throw new ValidationError("Problem ID is required");
        }
        ...
        const updatedProblem = await db.problem.update({
            ...
            data: {
                ...
                testcases: input.testcases ?? undefined,
                codeSnippets: input.codeSnippets ?? undefined,
                referenceSolutions: input.referenceSolutions ?? undefined,
            },
        });
     ...
     * Delete a problem
     */
    async deleteProblem(problemId, userId) {
        if (!problemId) {
            throw new ValidationError("Problem ID is required");
        }
        const problem = await db.problem.findUnique({
     ...
     */
    async getProblemsSolvedByUser(userId) {
     ...
     */
    async validateReferenceSolutions(testcases, referenceSolutions) {
        for (const [language, solutionCode] of Object.entries(referenceSolutions)) {
            ...
            try {
                ...
                for (let i = 0; i < testcases.length; i++) {
                    ...
                    if (!passed) {
                        throw new ValidationError(`Reference solution failed testcase ${i + 1} for ${language}. Expected: ${expectedOutput}, Got: ${actualOutput || result.status.description}`);
                    }
                }
            }
            catch (error) {
                ...
                throw new InternalServerError(`Failed to validate reference solution for ${language}`);
            }
        }
    }
}
