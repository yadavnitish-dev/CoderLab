...
import { buildBatchedStdin, executeSubmission, getLanguageName, } from "../libs/judge0.lib.js";
...
export class CodeExecutionService {
     ...
     */
    async executeCode(userId, input) {
        if (!userId) {
    ...
    async runExecution(input) {
        ...
        // Evaluate test cases
        let allPassed = true;
        const testResults = input.stdin.map((_, i) => {
    ...
    validateExecutionInput(input) {
        ...
        if (!Array.isArray(input.expected_outputs) ||
        ...
        if (input.stdin.length !== input.expected_outputs.length) {
            throw new ValidationError("Number of inputs must match number of expected outputs");
        }
        if (!input.problemId) {
