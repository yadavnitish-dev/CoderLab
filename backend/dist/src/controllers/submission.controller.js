...
export const getSubmissionsForProblem = async (req, res) => {
    try {
        ...
        const { problemId } = validateInput(req.params, z.object({ problemId: z.string().uuid("Invalid problem ID format") }));
        const submissions = await submissionService.getSubmissionsForProblem(req.user.id, problemId);
        res.status(200).json({
            success: true,
            message: "Submissions fetched successfully",
            submissions,
...
export const getAllTheSubmissionsForProblem = async (req, res) => {
    try {
        const { problemId } = validateInput(req.params, z.object({ problemId: z.string().uuid("Invalid problem ID format") }));
        const count = await submissionService.getSubmissionCountForProblem(problemId);
        res.status(200).json({
            success: true,
 ...
 */
function handleSubmissionError(error, res) {
