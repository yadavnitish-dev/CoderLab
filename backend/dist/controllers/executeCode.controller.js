...
import { buildBatchedStdin, executeSubmission, getLanguageName, parseBatchedStdout, } from "../libs/judge0.lib.js";
const splitExecutionOutput = (output) => {
...
export const executeCode = async (req, res) => {
    try {
        const { source_code, language_id, stdin, expected_outputs, problemId, mode = "run", } = req.body;
        if (!req.user) {
        ...
        let allPassed = true;
        const detailedResults = stdin.map((_, i) => {
